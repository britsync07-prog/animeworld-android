#!/usr/bin/env python3
"""
app.py -- runnable API server + static web host for the AnimeWorld clone.

Serves BOTH:
  * the REST API at  /api/v1/...   (proxies watchanimeworld.one)
  * the static site at /           (web/index.html, app.js, style.css)

Zero third-party deps (Python stdlib only). The site calls the API on the
same origin, so there are no CORS issues.

Run:
  python server/app.py [port]        # default 8000, binds 0.0.0.0
Then open http://127.0.0.1:8000/  (or the machine's LAN IP).
"""
import json
import mimetypes
import os
...
# only allows watchanimeworld.*), and the m3u8/segment CDNs send no CORS headers,
# so the browser can't embed or fetch them cross-origin. We proxy every playlist
# and segment through our own same-origin server and rewrite all URIs inside
# playlists so each one routes back through the proxy. This is what makes video
# actually play in the clone.
# ---------------------------------------------------------------------------
...
def _hls_fetch(target):
    """GET raw bytes for a playlist or segment (client._http only returns text)."""
...
def main():
    ...
    print(f"AnimeWorld server on http://{HOST}:{port}  (Ctrl+C to stop)")
